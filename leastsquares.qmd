...
`linalg_lstsqt()` で利用できる `driver` はLAPACKの異なる手続[^least_squares-1]に対応し、これらの手続は皆異なるアルゴリズムで問題を解く。
これは、今後行っていくことに類似している。

[^least_squares-1]: 上で引用した `driver` の説明は、基本的にLAPACKの[説明書](https://netlib.org/lapack/lug/node27.html) からの抜粋である。
...
この話題について一冊全体割いた本がいくつもあるので、詳細に立ち入ることは避ける。[^least_squares-2]

[^least_squares-2]: 詳しくは、そのような本の一つ、例えば、広く使われ（かつ簡明な） @Trefethen の記述を参照。

代わりに、悪条件な最小二乗問題の例を用いる。
つまり、行列が悪条件となっている。
この例を用いて、既に議論したアルゴリズムの数値安定性について理解する。[^least_squares-3]

[^least_squares-3]: この例は上の脚注にある @Trefethen の本から取った。[Computational Linear Algebra for Code](https://github.com/fastai/numerical-linear-algebra) （プログラマのための数値線型代数）の講義資料を通じて、この例を教えてくれたRachel Thomasに感謝する。
